...
public class Start {
	...
	for(CategoriaModel model:getCategorias()){
		...
		if(menordate == 0)menordate = lm.getDate().getTime();
		if(maiordate == 0)maiordate = lm.getDate().getTime();
	...
	if(categorias!=null){
		...
		CategoriaModel categoriaModel = new CategoriaModel(categoria.get("id").asText(),categoria.get("name").asText(), models);
	...
	File extratoFile = new File("/home/kevim/Downloads/extrato.txt");
